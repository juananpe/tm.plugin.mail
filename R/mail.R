convert_mbox_eml <- function(mbox, dir) {
    ...
    for (i in seq_along(content)) {
        if (length(grep("^From ", content[i])) > 0) {
            ...
            if (needWrite && start <= end) {
                con <- file(file.path(dir, counter))
    ...
    if (needWrite)
        writeLines(content[start:length(content)], file(file.path(dir, counter)))
}
...
removeMultipart.character <- function(x) {
    ...
    i <- grep("^Content-Type: text/plain", x)
    r <- character(0)
    k <- 2
    for (j in i) {
        ...
        r <- x(r, content[index:length(content)])
        k <- k + 1
    }
...
removeSignature.character <- function(x, marks = character(0)) {
    ...
    for (m in marks)
        signatureStart <- min(grep(m, x), signatureStart)
